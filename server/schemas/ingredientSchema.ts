...
/**
 * Schema for parsing raw ingredient text into structured components
 * Output from the LLM when parsing "2 cups green bell peppers, diced"
 */
...
/**
 * Schema for matching an ingredient name to an existing standardized ingredient
 * Output from the LLM when matching against existing ingredients
 */
export const ingredientMatchSchema = z.object({
  matchedId: z
    ...
    .nullable()
    .describe(
      'The ID of the matched ingredient from the database. Null if no good match found.'
    ),
  standardizedName: z
    .string()
    .describe(
...
/**
 * Schema for standardized ingredient data ready for database storage
 */
...
export type StandardizedIngredient = z.infer<
  typeof standardizedIngredientSchema
>;
